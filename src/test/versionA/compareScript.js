    ...
    altHighlightedComparedGraphics
        .on('mousemove', (event) => {
            if (interactionManager.hitTest(event.data.global, altHighlightedComparedGraphics)) {
                ...
                const data = clickedAltDataLine.data?.total?.[xIdentifier];
                let tooltipText = `${clickedAltDataLine.geo}<br>`;
                const roundedData = tooltipRoundData(data);
                tooltipText += `${xIdentifier} : ${roundedData || '-'}<br>`;
                if (!isNaN(roundedData) && altData[dataSetValue].dataColumnNames.length > 1) {
                    for (const columnData of altData[dataSetValue].dataColumnNames) {
                        tooltipText += `${columnData} : ${tooltipRoundData(clickedAltDataLine.data[columnData]?.[xIdentifier]) || '-'}<br>`;
                    }
                }
                tooltip.html(tooltipText);
                ...
                colorAltAreaOnHover(clickedAltDataLine, data, xIdentifier, true);
            }
        })
        .on('mouseout', () => {
            altHighlightedComparedGraphics.clear();
...
function drawAltComparedData() {
    ...
    for (const time in clickedAltDataLine.data?.total) {
        const data = clickedAltDataLine.data.total[time];
        const colorScale = getDiffOriColorScale(clickedAltDataLine.colorScale, data);
        altComparedGraphics.beginFill('0x' + d3.color(colorScale(data)).formatHex().substr(1));
        altComparedGraphics.drawRect(altInitialTimeScaleBand(time), 0, altInitialTimeScaleBand.bandwidth(), altComparedCanvasBox.height);
