...
/* Adds notifications when aggreMode or/and lassoMode is/are entered */
...
/* Adds Zoom to map for navigation */
    ...
    return d3.zoom()
        .on('zoom', (event) => {
            ...
            if (newScale !== sideElts[side].scale) {
                ...
                if (isNameDisplayed) checkLabelZoomDisplay(side);
                sideElts[side].scale = newScale;
            }
            if (isLineDisplayed) changeODLinesDisplay(side);
        })
        .scaleExtent([0.8, Infinity])
        .filter((event) => { return !event.ctrlKey && !event.shiftKey && !event.button; });
}

/* Adds Drag to map for lassoMode */
function addMapDragBehavior(side) {
    ...
    return d3.drag()
        ...
        .on('end', () => {
            if (!aggreMode) clearAreaSelection(side);
...
/* Checks if every area point of selection is selected in lasso */
...
/* Utility function for drag behaviour */
...
/**********************************************************************************************************************/
/* Handlers for top buttons (Clear and Fit) */

function addTopButtonHandlers(side) {
    d3.select(`#${side}Clear`).on('click', () => {
        clearAreaSelection(side);
...
/**********************************************************************************************************************/
/* Function that zooms to area path bounds when user click on od Data */

function showAreaOnMap(dataLine, color, side) {
    const areas = [];
    if (side === 'left') {
        if (isGroupByDestination) {
            for (const originCode of dataLine.originCode) {
                areas.push(d3.select(`#leftAreaPath_${removeLastDigits(originCode)}`));
            }
        } else {
            areas.push(d3.select(`#leftAreaPath_${removeLastDigits(dataLine.originCode)}`));
        }
    } else {
        if (isGroupByOrigin) {
            for (const destinationCode of dataLine.destinationCode) {
                areas.push(d3.select(`#rightAreaPath_${removeLastDigits(destinationCode)}`));
            }
        } else {
            areas.push(d3.select(`#rightAreaPath_${removeLastDigits(dataLine.destinationCode)}`));
        }
    }
    ...
    for (const area of areas) {
        ...
        switch (area.node().tagName) {
            case 'path':
                [[xx0, yy0], [xx1, yy1]] = sideElts[side].pathGenerator.bounds(area.data()[0]);
                break;
            case 'rect':
                const x = parseFloat(area.attr('x'));
                const y = parseFloat(area.attr('y'));
                const width = parseFloat(area.attr('width'));
                const height = parseFloat(area.attr('height'));
                [[xx0, yy0], [xx1, yy1]] = [[x, y], [x + width, y + height]];
                break;
            case 'circle':
                const cx = parseFloat(area.attr('cx'));
                const cy = parseFloat(area.attr('cy'));
                const r = parseFloat(area.attr('r'));
                [[xx0, yy0], [xx1, yy1]] = [[cx - r, cy - r], [cx + r, cy + r]];
                break;
        }
        x0 = Math.min(x0, xx0);
        y0 = Math.min(y0, yy0);
        x1 = Math.max(x1, xx1);
        y1 = Math.max(y1, yy1);
        if (color) area.style('fill', color);
    }
    const width = sideElts[side].svg.node().getBoundingClientRect().width;
    const height = sideElts[side].svg.node().getBoundingClientRect().height;
    if (isFinite(x0)) {
        sideElts[side].svg.transition().duration(fitDuration).call(
            sideElts[side].zoom.transform,
            d3.zoomIdentity
                .translate(width / 2, height / 2)
                .scale(0.9 * Math.min(width / (x1 - x0), height / (y1 - y0)))
...
/**********************************************************************************************************************/
/* Handlers for area mouse interaction */
...
function handleAreaEnter() {
    if (!lassoMode) {
        ...
        areaRaise(area);
        lastOpacity = areaName.style('opacity');
        applyStyle(areaName, {'fill': nameSelFontColor, 'font-weight': nameSelFontWeight, 'font-size': sideElts[side].nameCrtFontSize * nameFontSizeMult + 'px', 'opacity': 1});
...
function handleAreaLeave() {
    ...
    applyStyle(areaName, {'opacity': lastOpacity});
...
/**********************************************************************************************************************/
/* Functions used for area selection */
...
function applyAreaSelection(side, area, aggreMode, lassoMode) {
    ...
    if (aggreMode) {
        ...
        if (areaPath.classed('selected')) {
            sideElts[side].selectedAreas.unshift(areaPath.attr('id').split('_')[1]);
            areaRaise(area);
            ...
            sideElts[side].selectedAreas = sideElts[side].selectedAreas.filter(item => item !== areaPath.attr('id').split('_')[1]);
            if (lassoMode) {
                areaLower(area);
...
/* Deselects all areas */
function clearAreaSelection(side) {
...
function updateSelectedAreas(side) {
    ...
    d3.select(`#${side}SelectedAreasList tbody`).selectAll('tr')
        .data(selectedAreas)
        .join(
            enter => enter.append('tr').append('td').text(d => sideElts[side].areaData[d]?.Name || 'All'),
            update => update.text(d => sideElts[side].areaData[d]?.Name || 'All'),
            exit => exit.remove()
        );
...
function createAreaObject(side, areaPaths, areaNodes, areaShapes) {
    ...
    //d3.select(`#${side}Loading`).transition().duration(500).style('opacity', 0).on('end', function () {   //resize
        //d3.select(this).style('display', 'none');
        //d3.select(`#${side}Loading`).classed('fadeOut', true).classed('fadeIn', false);
...
function createMap(side, areaPaths, areaNodes, areaShapes) {
    ...
    d3.select(`#${side}TopButtonContainer`).style('opacity', 1);
    d3.select('#odLegend').style('opacity', 1);
    const firstG = sideElts[side].firstG;
    const areaData = sideElts[side].areaData;
    firstG.node().innerHTML = '';   //resize
