...
function addODZoom() {
    return d3.zoom()
        .on('zoom', (event) => {
            transformCanvas(event.transform);
...
async function bindData() {
    ...
    for (const oddataItem of oddataArray) {
        const data = oddataItem.data;
        oddataItem.x = odTimeline(oddataItem.date);
        oddataItem.y = odGeoLine(oddataItem.id);
        oddataItem.fillStyle = (data === 'n/a') ? 'grey' : colorScale(data);
    }
}

function createCanvas() {
    let msg = { eventType: 'createCanvas',
                canvas: offCanvas,
                height: canvas.attr('height'),
                width: canvas.attr('width'),
                devicePixelRatio: devicePixelRatio};
    const transfer = [offCanvas];
    worker.postMessage(msg, transfer);
    drawCanvas();
}

function drawCanvas() {
    console.log('odGeoLine.bandwidth() : ' + odGeoLine.bandwidth());
    console.log('odTimeline.bandwidth() : ' + odTimeline.bandwidth());
    const msg = {   eventType: 'drawCanvas',
                    oddata: oddataArray,
                    geoBandwidth: odGeoLine.bandwidth(),
                    timeBandwidth: odTimeline.bandwidth()};
    worker.postMessage(msg);
}

function transformCanvas(transform) {
    const msg = {   eventType: 'transformCanvas',
                    transform: transform};
    worker.postMessage(msg);
}
