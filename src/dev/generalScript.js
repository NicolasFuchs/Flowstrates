...
d3.select('#settingsLogo').on('click', () => {
    d3.select('#tabMap').node().click();
    d3.select('#settingsModal').style('display', 'flex');
    odHighlightedGraphics.interactive = false;
    altHighlightedGraphics.interactive = false;
});
d3.select('#sameMapDataLabel').on('click', function() {
    const isClicked = !d3.select(this).classed('clicked');
    d3.select(this).classed('clicked', isClicked);
});
d3.select('#displayAreaNamesLabel').on('click', function () {
    const isClicked = !d3.select(this).classed('clicked');
    d3.select(this).classed('clicked', isClicked);
    isNameDisplayed = !isNameDisplayed;
    d3.selectAll('.leftAreaName, .rightAreaName').style('display', (isNameDisplayed) ? 'block' : 'none');
    if (isNameDisplayed) {
        checkLabelZoomDisplay('left');
        checkLabelZoomDisplay('right');
    }
});
d3.select('#modalCancel').on('click', () => {
    d3.select('#settingsModal').style('display', 'none');
    odHighlightedGraphics.interactive = true;
    altHighlightedGraphics.interactive = true;
});
d3.select('#modalOK').on('click', () => {
    d3.select('#settingsModal').style('display', 'none');
    odHighlightedGraphics.interactive = true;
    altHighlightedGraphics.interactive = true;
});
d3.selectAll('.tab').on('click', function() {
    d3.selectAll('.tab').classed('clicked', false);
    d3.select(this).classed('clicked', true);
    d3.selectAll('.tabContent').style('display', 'none');
    d3.select(`#${d3.select(this).attr('id')}Content`).style('display', 'block');
});
